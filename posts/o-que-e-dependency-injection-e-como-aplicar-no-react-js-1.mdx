...
<Note>
  **Obs 1**: Outra forma seria utilizando a Context API. <br />
  Porém, a Context API **não é a solução para todos os casos de injeção de dependencias**. <br />
...
```tsx "components/header.tsx" showLineNumbers {4,5,10-12,14-17} title="components/header.tsx"
...
```tsx showLineNumbers {1-3,10} title="components/logo.tsx"
const Logo = () => {
  return (
    <S.LogoContainer>
      <img src="logo.svg" alt="Logo" />
...
```tsx showLineNumbers title="components/profile.tsx"
...
```ts showLineNumbers title="utils/get-item-localstorage.ts"
...
```tsx showLineNumbers {2-3,8,10} title="components/header.tsx"
const Header = () => {
